...
inverseBx = function(xmlist, Bmlist, ylist, theta, lamj, sig2, n, p, nu = 1)
{
  np = n*p
  Ip = 1/nu*diag(p)
  nIp = nu*n*diag(p)
  ...
  {
    Bmi = Bmlist[[i]]
    bmxi = cbind(xmlist[[i]], Bmi)
    ni = nrow(Bmi)
    Bthetai = Bmi %*% theta
    #Sigmainvi  = solve(sig2 * diag(ni) + Bthetai %*% diag(lamj) %*% t(Bthetai))
    Sigmainvi  = solve( diag(ni) + 1/sig2*Bthetai %*% diag(lamj) %*% t(Bthetai))
    mati = solve(t(bmxi) %*% Sigmainvi %*% bmxi + nIp)
    DB = DB + mati
    AB[idp1:idp2,] = mati
    matinv[idp1:idp2,idp1:idp2] = mati
    Bty[idp1:idp2] = t(bmxi) %*% Sigmainvi %*% ylist[[i]]
