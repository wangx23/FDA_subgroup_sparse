                   ...
                   maxiter = 50, tol = 1e-3)
{
  ...
  if(method == "mean")
  {
    alpm = do.call("rbind",by(betam0, group0, colMeans,simplify = TRUE))
    est = c(t(alpm))
  }
...
EMgroupv2 = function(ind, x, tm, y, knots, group0, P, betam0, boundary = c(0,1),
                   maxiter = 50, tol = 1e-3)
{
  ...
  Ux = cbind(x, Ux)  #### new Ux
  nx = ncol(x)
  
  est = solve(t(Ux)%*%Ux) %*% t(Ux) %*%y
  etahat = est[1:nx]
  alpm = matrix(est[-(1:nx)], ng, p, byrow = TRUE)
  ...
  for(m in 1:maxiter)
  {
    ...
    for(i in 1:n)
    {
      ...
      xmi = xmlist[[i]]
      Vi = solve(t(theta) %*% BtB[,,i] %*% theta/sig2 + Laminv)
      Btyi = t(Bmi) %*% (ylist[[i]] - xmi%*%etahat - Bmi%*%alpm[group0[i],])
      mi = 1/sig2 * Vi %*% t(theta) %*% Btyi
      Sigma = Sigma + mi %*% t(mi) + Vi
      residv[i] =  sum((ylist[[i]] - xmi %*%etahat - Bmi%*%alpm[group0[i],] - 
                          Bmi %*%theta %*% mi)^2) + sum(diag(Bmi %*% theta %*% Vi %*% t(theta) %*% t(Bmi)))
  ...
  for(i in 1:n)
  {
    residf[i] = sum((ylist[[i]] - xmlist[[i]]%*%etahat - 
                       Blist[[i]] %*% alpm[group0[i],] -
                       Blist[[i]] %*% theta %*% mhat[i,])^2)
  }
