...
"fclustinit" <-
  function(data, pert = 0, grid = seq(0.01, 1, length = 100), h = 1, p = 5, q = 5,K = K){
    S <- FullS <- NULL
    ...
    for (i in 1:N){
      # 2015/05/02 (Fan): try to incoporate individual with one observation only
      Si <- matrix(S[data$curve==i,],ncol=q)
...
"fclustEstep" <-
  function(parameters, data, vars, S, hard)
  {
    ...
    for(j in 1:N) {
      ...
      # 2015/05/02 (Fan): one-row matrix
      
      Sj <- S[data$curve == j,  ]
      if(is.null(dim(Sj))) Sj = matrix(Sj,nrow=1)
      
      nj <- sum(data$curve == j)
      
      
      # 2015/05/02 (Fan):
      invvar <- diag(1/rep(par$sigma,nj),nrow=nj)
...
"fclust.pred" <-
  function(fit,data=NULL,reweight=F)
  {
    ...
    for(i in 1:N) {
      ...
      # 2015/05/02 (Fan): one-row.
      if(is.null(dim(Sij))) Sij = matrix(Sij,nrow=1)
      
      xij <- data$x[curve == i]
      n <- length(xij)
      Sigma <- par$sigma * diag(n) + Sij %*% par$Gamma %*% t(Sij)
      # Calculate covariance for each alpha hat.
      InvCalpha <- t(par$Lambda) %*% t(Sij) %*% solve(Sigma) %*% Sij %*% par$Lambda
      
      # 2015/05/03: fix the sigular matrix problem
      Calpha.i <- try(solve(InvCalpha+diag(1e-16,nrow=nrow(InvCalpha)),tol=1e-50),silent=T)
      
      Calpha[i,  ,  ] = Calpha.i
      
      #     if(class(Calpha.i)!="try-error"){
      #         Calpha[i,  ,  ] = Calpha.i
      #     }else{
      #         Calpha[i,  ,  ] = solve(InvCalpha+diag(1e-16,nrow=nrow(InvCalpha)),tol=1e-50)
      #     }
...
"fclust.curvepred" <-
  function(fit, data=NULL, index=NULL, tau = 0.95, tau1 = 0.975)
  {
    ...
    for (i in index){
      ...
      # 2015/05/02 (Fan)
      if(is.null(dim(Si))) Si = matrix(Si,nrow=1)
      
      ni <- dim(Si)[1]
      invvar <- diag(1/rep(sigma, ni),nrow=ni)
